...
def CrossEntropy(y, t):
    return -t * torch.log(y) - (1 - t) * torch.log(1 - y)
...
#########################################################################
    ...
    h = torch.sum(inputMatrix[contextWords].t(), dim=1, keepdim=True)
    o = torch.mm(outputMatrix, h)
    y = softmax(o)

    loss = -torch.log(y[centerWord])
