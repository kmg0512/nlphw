...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    #############################################################################
    h = torch.max(X.mm(W1).add(b1), torch.Tensor([[0]]))
    scores = h.mm(W2).add(b2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #       'y'는 정답 index를 가리키며 해당하는 확률에 -log 적용하여 평균        #
    #############################################################################
    loss = -softmax[range(y.size(0)),y].log().mean()
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    softmax_grad[range(y.size(0)),y]-=1
    softmax_grad/=scores.size(0)
    grads['W2'] = h.t().mm(softmax_grad)
    grads['b2'] = softmax_grad.sum(0)
    h_grad = softmax_grad.mm(W2.t())*(h>0).float()
    grads['W1'] = X.t().mm(h_grad)
    grads['b1'] = h_grad.sum(0)
